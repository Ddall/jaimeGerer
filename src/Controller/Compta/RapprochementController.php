...
use App\Entity\Compta\Rapprochement;
use App\Util\DependancyInjectionTrait\JournalBanqueTrait;
...
class RapprochementController extends AbstractController
{
    ...
    use JournalBanqueTrait;


    /**
    * @Route("/compta/rapprochement", name="compta_rapprochement_index")
    */
    public function rapprochementIndexAction(Request $request)
    {
        ...
        $formBuilder->add('comptes', EntityType::class, array(
            'required' => true,
            'class' => 'App:Compta\CompteBancaire',
            'label' => 'Compte bancaire',
            'choices' => $arr_comptesBancaires,
            'attr' => array('class' => 'compte-select'),
           // 'data' => $this->getDoctrine()->getManager()->getReference("App:Compta\CompteBancaire", $filtreReleveBancaire['id'])
        ));
        ...
        //remises de cheque
        $repoRemiseCheque = $this->getDoctrine()->getManager()->getRepository('App:Compta\RemiseCheque');
        ...
        //factures
        $repoFactures = $this->getDoctrine()->getManager()->getRepository('App:CRM\DocumentPrix');
        ...
        //depenses
        $repoDepenses = $this->getDoctrine()->getManager()->getRepository('App:Compta\Depense');
        ...
        //notes de frais
        $repoNotesFrais = $this->getDoctrine()->getManager()->getRepository('App:NDF\NoteFrais');
        ...
        //accomptes
        $repoAccomptes = $this->getDoctrine()->getManager()->getRepository('App:Compta\Accompte');
        ...
        //avoirs fournisseurs
        $repoAvoirs = $this->getDoctrine()->getManager()->getRepository('App:Compta\Avoir');
        ...
        $repoAffectations = $this->getDoctrine()->getManager()->getRepository('App:Compta\AffectationDiverse');
   ...
   */
  public function rapprochementVoirAction(Request $request)
  {
    ...
    $arr_filtres = $request->request->get('filtres');
    $arr_filtres['id'] = $id;
    $session = $request->getSession();
    $session->set('FiltreReleveBancaire', $arr_filtres);

    //affichage de la liste des mouvements bancaires du compte
    $mouvementRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\MouvementBancaire');
    ...
    */
    public function rapprochementSupprimerAction(Rapprochement $rapprochement)
    {
        ...
        $journalBanqueRepo = $em->getRepository('App:Compta\JournalBanque');
  ...
  public function mouvementBancaireRapprocherAction(MouvementBancaire $mouvementBancaire, $type, $piece)
  {
    ...
    switch($type){
        case 'DEPENSE' :
            $repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\Depense');
      ...
      case 'FACTURE' :
        $repo = $this->getDoctrine()->getManager()->getRepository('App:CRM\DocumentPrix');
      ...
      case 'ACCOMPTE' :
        $repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\Accompte');
      ...
      case 'AVOIR-FOURNISSEUR' :
        $repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\Avoir');
      ...
      case 'REMISE-CHEQUES' :
        $repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\RemiseCheque');
      ...
      case 'NOTE-FRAIS' :
        $repo = $this->getDoctrine()->getManager()->getRepository('App:NDF\NoteFrais');
    ...
    try{
      $this->journalBanqueService->journalBanqueAjouterAction($type, $rapprochement);
      $em->flush();
  ...
  // {
  //   $em = $this->getDoctrine()->getManager();
  //   $mouvementsRepo = $em->getRepository('App:Compta\MouvementBancaire');
  ...
  //   $formBuilder = $this->createFormBuilder();
  //   $formBuilder ->add('fusion', \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
  //               'required' => true,
  ...
  /**
    ...
    */
    public function rapprochementAvanceAction()
    {
        ...
        $remiseChequeRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\RemiseCheque');
        $factureRepo = $this->getDoctrine()->getManager()->getRepository('App:CRM\DocumentPrix');
        $depenseRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\Depense');
        $noteFraisRepo = $this->getDoctrine()->getManager()->getRepository('App:NDF\NoteFrais');
        $avoirRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\Avoir');
        $affectationDiverseRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\AffectationDiverse');
     ...
     */
    public function rapprochementAvanceRapprocherAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $mouvementBancaireRepo = $em->getRepository('App:Compta\MouvementBancaire');

        $arr_mouvementsId = $request->request->get('mouvements');
        ...
        $arr_piecesId = $request->request->get('pieces');
        ...
        foreach($arr_mouvementsId as $mouvementId){
            ...
            foreach($arr_piecesId as $arr_piece){
                ...
                switch($type){
                  case 'DEPENSES' :
                    $repo = $em->getRepository('App:Compta\Depense');
                    $piece = $repo->find($id);
                  ...
                  case 'FACTURES' :
                    $repo = $em->getRepository('App:CRM\DocumentPrix');
                    $piece = $repo->find($id);
                  ...
                  case 'AVOIRS-FOURNISSEUR' :
                    $repo = $em->getRepository('App:Compta\Avoir');
                    $piece = $repo->find($id);
                  ...
                  case 'REMISES-CHEQUES' :
                    $repo = $em->getRepository('App:Compta\RemiseCheque');
                    $piece = $repo->find($id);
                    $rapprochement->setRemiseCheque($piece);
                    break;

                case 'AFFECTATIONS-DIVERSES-VENTE' :
                    $repo = $em->getRepository('App:Compta\AffectationDiverse');
                    $piece = $repo->find($id);
                ...
                case 'AFFECTATIONS-DIVERSES-ACHAT' :
                  ...
                  case 'NOTES-FRAIS' :
                    $repo = $em->getRepository('App:NDF\NoteFrais');
                    $piece = $repo->find($id);
        ...
        try {
            $this->journalBanqueService->journalBanqueAjouterPlusieursPiecesMemeCompteAction($arr_mouvements, $arr_pieces);
            $em->flush();
