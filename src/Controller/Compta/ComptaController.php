...
class ComptaController extends AbstractController
{
	 ...
	 */
	public function indexAction(ChartService $chartService, OpportuniteService $opportuniteService)
	{
	 ...
	 */
	public function activationReportsANouveauImporterUploadAction(Request $requestData){
	...
	public function activationAction(){
		...
		$affDivRepo = $em->getRepository('App:Compta\AffectationDiverse');
	 ...
	 */
	public function activationCompteTVAAchatsAction(Request $request, $prev){
		...
		$formBuilder
			->add('compteComptableTVA', \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
            		'choices' => $arr_choices_tva,
            		'required' => true,
            		'label' => 'Compte de TVA sur achats à utiliser',
					'data' => $choice
         	))
			->add('compteComptableNoTVA', \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
	 ...
	 */
	public function activationAnalytiqueTVAAction(Request $request){
		...
		foreach($arr_analytiques as $analytique){
			if($analytique->getValeur()){
				$formBuilder->add('analytique-'.$analytique->getId(), \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
					'choices' => $arr_choice,
	...
	public function activationFactureImporterAction(){
		...
		$setActRepo = $this->getDoctrine()->getManager()->getRepository('App:SettingsActivationOutil');
		...
		//CRM activée
// 		if($crmActive != null){
// 			$documentPrixRepo = $this->getDoctrine()->getManager()->getRepository('App:CRM\DocumentPrix');
// 			$count = $documentPrixRepo->custom_count($this->getUser()->getCompany(), 'FACTURE');
...
// 		}
	 ...
	 */
	public function activationProduitsAction(Request $request, $prev){
		...
		foreach($arr_produits as $produit){
			if($produit->getValeur()){
				$formBuilder->add($produit->getId(), \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
	...
	// 	$numService = $this->get('appbundle.num_service'); // @upgrade-notes <- this is legacy code
