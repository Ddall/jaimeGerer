...
class FactureController extends AbstractController
{
	...
	// 	$analytiqueRepo =  $this->getDoctrine()->getManager()->getRepository('App:Settings');
	 ...
	 * )
	 */
	public function factureListeAjaxAction(Request $requestData)
	{
		...
		$response->setData(array(
				...
				'recordsTotal' => $repository->custom_count($this->getUser()->getCompany(), 'FACTURE', null, null, true),
	 ...
	 * )
	 */
	public function factureTotalAjaxAction(Request $request){
	 ...
	 * )
	 */
	public function factureListeRetardAjaxAction(Request $requestData)
	{
		$arr_sort = $requestData->get('order');
		$arr_cols = $requestData->get('columns');
	 ...
	 * )
	 */
	public function factureListeEcheanceAjaxAction(Request $requestData)
	{
		$arr_sort = $requestData->get('order');
		$arr_cols = $requestData->get('columns');
	 ...
	 */
	public function factureRelancerEmailAction(Request $request, $id)
	{
		...
		$relanceRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\Relance');
		...
		$form->add('addcc', CheckboxType::class, array(
				'required' => false,
	 ...
	 * )
	 */
	public function factureRappelerEcheanceEmailAction(Request $request, DocumentPrix $facture)
	{
		//formulaire de redaction du mail
		$form = $this->createFormBuilder()->getForm();
		$form->add('objet', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
	 ...
	 * )
	 */
	public function factureRelancerCourrierAction(Request $request, MailevaService $mailEvaService, $id)
	{
		$factureRepo = $this->getDoctrine()->getManager()->getRepository('App:CRM\DocumentPrix');
		...
		$form->add('recommande', CheckboxType::class, array(
				'required' => false,
				'disabled' => $disabled,
	 ...
	 */
	public function factureImporterHistoriqueUploadAction(Request $requestData)
	{
		$em = $this->getDoctrine()->getManager();
	 ...
	 */
	public function factureImporterHistoriqueAction(Request $request)
	{
		$form = $this->createForm(UploadHistoriqueFactureType::class); // @upgrade-note removed unused parameter $this->getUser()->getCompany()
	 ...
	 */
	public function factureImporterMappingAction(Request $request, $initialisation = false)
	{
		$session = $request->getSession();
...
// 		}
        ...
        $form_mapping = $this->createForm(UploadHistoriqueFactureMappingType::class, null, array(
	 ...
	 */
	public function factureImporterValidationAction(Request $request, $initialisation = false)
	{
		$session = $request->getSession();
		...
		if(count($arr_err_comptes) != 0){
			...
			foreach($arr_err_comptes as  $key => $err_compte){
				$formBuilder->add('compte-'.$key, \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
	 ...
	 */
	public function factureImporterExecutionAction(Request $request, JournalVentesController $journalVenteService, $initialisation = false)
	{
		$session = $request->getSession();
	 ...
	 */
	public function factureChoisirAction(Request $request, $initialisation = false)
	{
		$em = $this->getDoctrine()->getManager();
