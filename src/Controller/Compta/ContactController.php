...
use App\Form\Compta\ContactType;
...
class ContactController extends AbstractController
{
	 ...
	 * )
	 */
	public function contactListeAjaxAction(Request $requestData, $type, $year)
	{
		...
		if($type == "CLIENT"){
			...
			$factureRepository = $this->getDoctrine()->getManager()->getRepository('App:CRM\DocumentPrix');
			...
			$depenseRepository = $this->getDoctrine()->getManager()->getRepository('App:Compta\Depense');
			for($i=0; $i<count($list); $i++){
	 ...
	 */
	public function comptaContactAjouterAction(Request $request, $type)
	{
		...
		$form = $this->createForm(ContactType::class, $compte, array(
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if($type == 'CLIENT' || $both){
				...
				$compteComptable = $this->compteComptableService->createCompteComptableForCompte('411', $compte->getNom());
	 ...
	 */
	public function comptaContactAjouterExistantAction(Request $request, $type)
	{
		$em = $this->getDoctrine()->getManager();
		$ccRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\CompteComptable');
		...
		$builder->add('compte_name', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
			'required' => true,
			'label' => 'Organisation',
	 ...
	 * )
	 */
	public function comptaContactEditerAction(Request $request, Compte $compte, $type)
	{
		$ccRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\CompteComptable');
