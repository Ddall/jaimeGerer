...
use App\Form\Compta\LigneJournalCorrectionType;
use App\Service\Compta\CompteComptableService;
use App\Service\Compta\OperationDiverseService;
...
class CompteComptableController extends AbstractController
{
	 ...
	 */
	public function compteListeAction(Request $request)
	{
		$repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\CompteComptable');
		$form = $this->createFormBuilder()->getForm();

		$form->add('recherche', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
			'label' => 'Rechercher',
			'required' => true,
	...
	public function compteUtiliserGeneralAction($initialisation = false)
	{
		...
		$ccRepo = $em->getRepository('App:Compta\CompteComptable');
	 ...
	 */
	public function compteImporterUploadAction(Request $requestData)
	{
	 ...
	 */
	public function compteImporterMappingClientsFournisseursAction(Request $request, $initialisation){
		...
		foreach($arr_clients as $key => $compteClient){
			...
			$formBuilder->add($compteClient.'-radio', \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
					'required' => true,
					'mapped' => false,
					'expanded' => true,
					'choices' => array(
			...
			->add($compteClient.'-name', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
 					'required' => false,
					'mapped' => false,
					'label' => $compteClient,
			...
			->add($compteClient.'-compte', \Symfony\Component\Form\Extension\Core\Type\HiddenType::class, array(
 					'required' => false,
					'attr' => array('class' => 'entity-compte'),
					'data' => $dataCompte,
			))
			->add($compteClient.'-label', \Symfony\Component\Form\Extension\Core\Type\HiddenType::class, array(
					'required' => false,
					'attr' => array('class' => 'entity-compte'),
					'data' => $key,
			));
		...
		foreach($arr_fournisseurs as $key => $compteFournisseur){
			...
			$formBuilder->add($compteFournisseur.'-radio', \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
					'required' => true,
					'mapped' => false,
					'expanded' => true,
					'choices' => array(
			...
			->add($compteFournisseur.'-name', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
					'required' => false,
					'mapped' => false,
					'label' => 'Fournisseur',
					'attr' => array('class' => 'typeahead-compte'),
			...
			->add($compteFournisseur.'-compte', \Symfony\Component\Form\Extension\Core\Type\HiddenType::class, array(
					'required' => false,
					'attr' => array('class' => 'entity-compte'),
					'data' => $dataCompte,
			))
			->add($compteFournisseur.'-label', \Symfony\Component\Form\Extension\Core\Type\HiddenType::class, array(
					'required' => false,
					'attr' => array('class' => 'entity-compte'),
					'data' => $key,
			));
	 ...
	 */
	public function compteImporterAction(Request $request, $initialisation = false)
	{
		$form = $this->createFormBuilder()->getForm();

		$form->add('fichier', FileType::class, array(
				'required' => true,
				'label' => 'Fichier Excel',
		));
	 ...
	 */
	public function compteAjouterAction(Request $request)
	{
		$cc = new CompteComptable();
		$cc->setCompany($this->getUser()->getCompany());
		$form = $this->createForm(CompteComptableType::class, $cc);
	 ...
	 */
	public function compteAjouterSousCompteAction(Request $request, CompteComptable $compte, $redirect=0){
	 ...
	 */
	public function compteSupprimerSousCompteAction(Request $request, CompteComptable $compte){
	 ...
	 */
	public function compteComptableCorrigerAction(OperationDiverseService $operationDiverseService, CompteComptableService $compteComptableService, Request $request, $id, $codeJournal, $redirectRoute = null){
		...
		switch($codeJournal){
			...
			case 'VE':
				$repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\JournalVente');
				break;
			...
			default:
				$repo = $this->getDoctrine()->getManager()->getRepository('App:Compta\JournalBanque');
				$banque = true;
				break;
		...
		$form = $this->createForm(LigneJournalCorrectionType::class, null, array(
	 ...
	 */
	public function compteCorrigerLettrageAction(Request $requestData, $id, $journal){
		$response = new Response();

		$em = $this->getDoctrine()->getManager();
		$repo = $em->getRepository('App:Compta\JournalBanque');
		if($journal == 'VE'){
			$repo = $em->getRepository('App:Compta\JournalVente');
		} else if($journal == 'AC'){
			$repo = $em->getRepository('App:Compta\JournalAchat');
		} else if($journal == 'OD'){
			$repo = $em->getRepository('App:Compta\OperationDiverse');
		}
	 ...
	 */
	public function compteCommenterAction(Request $requestData, $id, $journal){
		$response = new Response();
