...
use App\Entity\Compta\JournalBanque;
use App\Entity\Compta\Rapprochement;
use App\Form\Compta\JournalBanqueCorrectionType;
use App\Util\DependancyInjectionTrait\LettrageServiceTrait;
...
class JournalBanqueController extends AbstractController
{

    use NumServiceTrait;
    use LettrageServiceTrait;
	...
	public function journalBanqueAjouterPlusieursPiecesAction($arr_mouvements, $arr_pieces){
		...
		$em = $this->getDoctrine()->getManager();
		$journalVenteRepo = $em->getRepository('App:Compta\JournalVente');
		$journalAchatRepo = $em->getRepository('App:Compta\JournalAchat');
		...
		try{
			switch($type){
				...
				case 'FACTURES':
					...
					$lettre = $this->lettrageService->findNextNum($piece->getCompte()->getCompteComptableClient());
				...
				case 'DEPENSES':
					...
					$lettre = $this->lettrageService->findNextNum($piece->getCompte()->getCompteComptableFournisseur());
					$lettrage = $prefixe.$lettre;
				...
				case 'NOTES-FRAIS':
					...
					$lettre = $this->lettrageService->findNextNum($piece->getCompteComptable());
					$lettrage = $prefixe.$lettre;
				...
				case 'AVOIRS-FOURNISSEUR':
					...
					$lettre = $this->lettrageService->findNextNum($piece->getDepense()->getCompte()->getCompteComptableFournisseur());
					$lettrage = $prefixe.$lettre;
				...
				case 'AVOIRS-CLIENT':
					...
					$lettre = $this->lettrageService->findNextNum($piece->getFacture()->getCompte()->getCompteComptableClient());
					$lettrage = $prefixe.$lettre;
	...
	public function journalBanqueExporterAction(CompteBancaire $compteBancaire, $year){
		...
		foreach($arr_journalBanque as $ligne){
			...
			$objPHPExcel->getActiveSheet ()->setCellValue ($col.$row, (string)$libelle);
			$col++;
	...
	private function ajouterFacture($mouvementBancaire, $facture, $ecrireLigneBanque = true, $modePaiement = null, $numEcriture = null){
		...
		try{
			...
			$lettrage = $this->lettrageService->findNextNum($facture->getCompte()->getCompteComptableClient());
	...
	private function ajouterDepense($mouvementBancaire, $depense, $ecrireLigneBanque = true, $noteFrais = null){
		...
		try{
			...
			//récupération du prochain numéro de lettrage pour le compte comptable client
			if($noteFrais){
				$lettrage = $this->lettrageService->findNextNum($noteFrais->getCompteComptable());
			} else {
				$lettrage = $this->lettrageService->findNextNum($depense->getCompte()->getCompteComptableFournisseur());
			}
	...
	private function ajouterAvoirFournisseur($mouvementBancaire, $avoir, $ecrireLigneBanque = true, $modePaiement = null){
		...
		try{
			...
			//récupération du prochain numéro de lettrage pour le compte comptable client
			$lettrage = $this->lettrageService->findNextNum($avoir->getDepense()->getCompte()->getCompteComptableFournisseur());
	...
	private function ajouterAvoirClient($mouvementBancaire, $avoir, $ecrireLigneBanque = true){
		...
		try{
			...
			//récupération du prochain numéro de lettrage pour le compte comptable client
			$lettrage = $this->lettrageService->findNextNum($avoir->getFacture()->getCompte()->getCompteComptableClient());
	...
	private function ajouterOD($mouvementBancaire, $od, $ecrireLigneBanque = true, $modePaiement = null){
		...
		try{
			...
			//récupération du prochain numéro de lettrage pour le compte comptable client
			$lettrage = $this->lettrageService->findNextNum($od->getCompteComptable());
	...
	// 	$rapprochementRepo = $em->getRepository('App:Compta\Rapprochement');
	// 	$compteBancaireRepo = $this->getDoctrine()->getManager()->getRepository('App:Compta\CompteBancaire');
	// 	$journalBanqueService = $this->get('appbundle.compta_journal_banque_controller');
	...
	// 	$journalAchatRepo = $em->getRepository('App:Compta\JournalAchat');
	// 	$rapprochementRepo = $em->getRepository('App:Compta\Rapprochement');
	// 	$this->lettrageService = $this->get('appbundle.compta_lettrage_service');
	...
	// 			$lettrage = $this->lettrageService->findNextNum($cc);
