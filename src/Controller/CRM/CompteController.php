...
use App\Entity\CRM\CompteRepository;
use App\Form\CRM\CompteFusionnerType;
use App\Form\CRM\CompteImportMappingType;
use App\Form\CRM\CompteImportType;
use App\Form\CRM\CompteType;
...
class CompteController extends AbstractController
{
	 ...
	 */
	public function compteListeAjaxAction(Request $requestData)
	{
		...
		$repositoryContact = $this->getDoctrine()->getManager()->getRepository('App:CRM\Contact');
	...
	public function compteAjouterAction(Request $request)
	{
		...
		$form = $this->createForm(CompteType::class, $compte, array(
		    'userGestionId' => $compte->getUserGestion()->getId(),
            'companyId' => $this->getUser()->getCompany()->getId(),
        ));

		$form->add('addressPicker', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
	 ...
	 */    
    public function compteFusionnerRecapitulatifAction(Request $request, CompteService $compteService)
    {
        if((null !== $idCompteA = $request->get('idCompteA')) && (null !== $idCompteB = $request->get('idCompteB'))){
            ...
            if($compteA && $compteB && $compteA->getCompany() === $this->getUser()->getCompany() && $compteB->getCompany() === $this->getUser()->getCompany()){
                ...
                $compteFusionnerForm = $this->createForm(CompteFusionnerType::class, $compteA, array(
                    'compteA' => $compteA,
                    'compteB' => $compteB,
                ));
	 ...
	 */
	public function compteAjouterModalAction(Request $request)
	{
		$compte = new Compte();
		$compte->setUserGestion($this->getUser());
		$compte->setCompany($this->getUser()->getCompany());
        ...
        $form = $this->createForm(CompteType::class, $compte, array(
            'userGestionId' => $compte->getUserGestion()->getId(),
            'companyId' => $this->getUser()->getCompany()->getId(),
            'action' => $this->generateUrl('crm_compte_ajouter_modal')
        ));

		$form->add('addressPicker1', \Symfony\Component\Form\Extension\Core\Type\TextType::class, array(
			'label' => 'Veuillez renseigner l\'adresse ici',
			'mapped' => false,
			'required' => false,
		));
	 ...
	 */
	public function compte_list_fusionnerAction(Request $request, $compte_id)
	{
		//~ echo "hich"; exit;
	 ...
	 */
	public function compteEditerAction(Request $request, Compte $compte)
	{
		$em = $this->getDoctrine()->getManager();
		$contactRepository = $em->getRepository('App:CRM\Contact');
	 ...
	 */
	public function compteSupprimerAction(Request $request, Compte $compte)
	{
		$em = $this->getDoctrine()->getManager();
		$contactRepo = $em->getRepository('App:CRM\Contact');
		$documentPrixRepo = $em->getRepository('App:CRM\DocumentPrix');
	...
	public function compte_contacts_listAction() {
		...
		$compteRepository = $this->getDoctrine()->getManager()->getRepository('App:CRM\Compte');
	 ...
	 */
	public function compteImporterAction(Request $request)
	{
		$form = $this->createForm(CompteImportType::class); // @upgrade-note removed unused parameter $this->getUser()->getCompany()
	 ...
	 */
	public function compteImporterMappingAction(Request $request)
	{
		$session = $request->getSession();
		...
		$form_mapping = $this->createForm(CompteImportMappingType::class, null, array(
	 ...
	 */
	public function compteVerifierBounceAction(ContactService $contactService, Compte $compte)
	{
		$contactRepository = $this->getDoctrine()->getManager()->getRepository('App:CRM\Contact');
		$arr_contacts = $contactRepository->findByCompte($compte);
