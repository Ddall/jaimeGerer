...
use App\Form\CRM\ProspectionImporterMappingType;
...
class ProspectionController extends AbstractController
{
	 ...
	 */
	public function prospectionImporterUploadAction(Request $request, Prospection $prospection)
	{
	 ...
	 */
	public function prospectionImporterAction(Request $request, Prospection $prospection)
	{
		$session = $request->getSession();
	 ...
	 */
	public function prospectionImporterMappingAction(Request $request, Prospection $prospection)
	{
		//~ exit;
		$session = $request->getSession();
		...
		$form_mapping = $this->createForm(ProspectionImporterMappingType::class, null, array(
		    'arr_headers' => $headers,
            'filename' => $files
        ));
...
//
	 ...
	 */
	public function contactImporterValidationAction(Request $request, Prospection $prospection)
	{
		//~ echo 1; exit;
		$session = $request->getSession();
				...
				switch ($entite){
					case 'contact' :
						foreach( $file as $k=>$v )
						{
							...
							$formBuilder->add($entite.'-'.$indexFile.'-'.$key.'-radio', \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class, array(
									'required' => true,
									'mapped' => false,
									'expanded' => true,
									'choices' => array(
