...
class TableauTVAService {

  protected $logger;
  protected $em;
  protected $utilsService;

  public function __construct(LoggerInterface $tvaLogger, EntityManagerInterface $em, UtilsService $utilsService)
  {
    $this->logger = $tvaLogger;
    $this->em = $em;
  ...
  public function creerTableauTVA($company, $year){
		...
		*/
		...
		// $settingsActivationRepo = $this->em->getRepository('App:SettingsActivationOutil');
		$documentPrixRepo = $this->em->getRepository('App:CRM\DocumentPrix');
		$depenseRepo = $this->em->getRepository('App:Compta\Depense');
		$rapprochementsRepo = $this->em->getRepository('App:Compta\Rapprochement');
				...
				foreach($arr_rapprochements as $rapprochement){
					if($rapprochement->getDepense()){
						...
						if($rapprochement->getDepense()->getAnalytique()->getNoTVA()){
							...
							$taxePercent = 0;
							if(round($depense->getTotalHT() * 0.2, 2) == $depense->getTotalTVA()){
								$taxePercent = 200;
							} else if(round($depense->getTotalHT() * 0.055, 2) == $depense->getTotalTVA()){
								$taxePercent = 55;
							} else if(round($depense->getTotalHT() * 0.1, 2) == $depense->getTotalTVA()){
								$taxePercent = 100;
							}

							if(array_key_exists($taxePercent, $arr_soumis[$type]['taxe_percent'])){
