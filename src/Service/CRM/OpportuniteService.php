...
class OpportuniteService {
    ...
    /**
     * OpportuniteService constructor.
     *
    ...
    public function findOpportunitesSousTraitancesAFacturer($company){
        $repo = $this->em->getRepository('App:CRM\OpportuniteSousTraitance');
    ...
    public function getTauxTransformationData($company, $year){
        ...
        $repo = $this->em->getRepository('App:CRM\Opportunite');
    ...
    public function getDataChartTempsCommercialParMontant($company, $year){
        ...
        foreach($tranches as $label => $tranche){
            ...
            if(null == $montantMin[1]){
                ...
                $montantMin = floatval($montantMin[1]);
            }

            if(null == $montantMax[1]){
                $montantMax = 0;
            } else {
                $montantMax = floatval($montantMax[1]);
            }
