...
class ContactService
{
     ...
     * @param ZeroBounceAPIService   $zeroBounceAPIService
     * @param TokenStorageInterface  $tokenStorage
     * @param LoggerInterface        $logger
     */
    public function __construct(string $rootDir = '', EntityManagerInterface $em, RequestStack $requestStack, ZeroBounceAPIService $zeroBounceAPIService, TokenStorageInterface $tokenStorage, LoggerInterface $logger)
    {
        $this->em = $em;
    ...
    public function checkContactImportFile($company)
    {
        ...
        $compteRepo = $this->em->getRepository('App:CRM\Compte');
        ...
        switch ($mode) {
            ...
            case self::MERGE_MODE_DOUBLON:
                if ($contactA->getCompte() !== $contactB->getCompte()) {
                    if (($contactA->getDocumentsPrix()->custom_count() > 0 || $contactA->getOpportunites()->custom_count() > 0) && ($contactB->getDocumentsPrix()->custom_count() > 0 || $contactB->getOpportunites()->custom_count() > 0)) {
