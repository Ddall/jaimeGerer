...
{
	public function custom_count($company, $type, $SettingsRepository = null, $DevisParent = null, $compta=null){
	...
	public function createQueryAndGetResult($arr_filters, $type, $company){
		...
		foreach($arr_filters as $filter){
			...
			if (strpos($champ,'|')) {
				...
				$_table = "App\\Entity\CRM\\".$arr_table[0];
		...
		$query->leftJoin('App\Entity\CRM\Compte', 'co', 'WITH', 'co.id = c.compte')
	...
	public function findNoRapprochement($company, $compta=null){
		...
		->from('App\Entity\Compta\JournalVente', 'j')
	...
	public function findForRemiseCheque($company, $type, $compta = null){
		...
		->from('App\Entity\Compta\JournalVente', 'jv')
	...
	public function findForListRetard($company, $type, $length, $start, $orderBy, $dir, $search, $compta=null){
		...
		$query->select('d.id', 'd.objet', 'd.num', 'd.dateCreation', 'd.dateValidite', 'c.nom as compte_nom', 'c.id as compte_id', 're.num as num_relance')
			->leftJoin('App\Entity\CRM\Compte', 'c', 'WITH', 'c.id = d.compte')
			->leftJoin('App\Entity\Compta\Relance', 're', 'WITH', 'c.id = re.facture')
			->where('c.company = :company')
